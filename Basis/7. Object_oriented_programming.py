# 编程之中更多利用函数，编写函数与调用函数，函数比较重要，面向对象好理解，面向对象编程是对现实的模拟，面向对象的目的，就是用代码来模拟世界，模拟现实世界的事物情景，让代码的编写，组织，理解更加容易
# 类和对象：类即class是描述一类事物，相当于一个事物模板，而对象则是一个具体的类的实例，比如人是一个类，实例化成对象即李明，王五等，类是比函数更大的范围的实体
# 编写一个类
class Student:  #定义类的关键字是class，类的名字一般是驼峰表示法（惯例），即大小写交错，注重结尾冒号
# 上方即一个学生的类
# 下方就是函数了
